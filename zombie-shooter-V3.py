...
# Bosses
boss_size = 30
boss_speeds = {2: 0.8, 3: 1.2}
boss_max_healths = {2: 100, 3: 150}
boss_damage = {'pistol': 2, 'ar': 5}

# Bullets
bullet_speeds = {'pistol': 10, 'ar': 15}
fire_rates = {'pistol': 0, 'ar': 100}

# Weapon pickup
pickup_range = 100
weapon_pickups = [{'type': 'ar', 'pos': [WIDTH * 0.75, HEIGHT - 20], 'size': [30, 10]}]
...
def draw_boss(boss, walk_cycle, level):
    head_radius = 10
    body_length = 25
    arm_length = 15
    leg_length = 20
    color = BOSS_GREEN if level == 2 else BOSS_PURPLE
    ...
    health_bar_width = 50
    health_bar_height = 5
    health_ratio = health / boss_max_healths[level]
...
def draw_weapon_pickups(weapon_pickups, player_pos):
    for weapon in weapon_pickups:
        pickup_rect = ar_sprite.get_rect(center=weapon['pos'])
        screen.blit(ar_sprite, pickup_rect.topleft)
...
def menu_screen():
    ...
    level1_button = pygame.Rect(WIDTH//2 - 100, HEIGHT//2 + 80, 200, 50)
    level2_button = pygame.Rect(WIDTH//2 - 100, HEIGHT//2 + 140, 200, 50)
    level3_button = pygame.Rect(WIDTH//2 - 100, HEIGHT//2 + 200, 200, 50)
    title_text = big_font.render("Zombie Shooter", True, WHITE)
    instruction_text = font.render("Enter Name and Select Level", True, WHITE)
    ...
    level3_text = font.render("Level 3 (Locked)", True, GRAY)
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif level1_button.collidepoint(event.pos) and player_name.strip():
                    return player_name.strip(), 1
...
def main():
    ...
    while True:
        ...
        weapon_type = 'pistol'
        last_shot_time = 0
        weapon_pickups_reset = [{'type': 'ar', 'pos': [WIDTH * 0.75, HEIGHT - 20], 'size': [30, 10]}]
        level = start_level
